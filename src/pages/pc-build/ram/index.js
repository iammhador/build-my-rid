...
import { addRam } from "@/redux/features/products/productSlice";
import Image from "next/image";
import Link from "next/link";
import { useDispatch } from "react-redux";

function RamBuilder({ ram }) {
  const dispatch = useDispatch();

  const handleAddToBuild = (ram) => {
    dispatch(addRam(ram));
  };
  return (
...
export default RamBuilder;

RamBuilder.getLayout = function getLayout(page) {
  return <RootLayout>{page}</RootLayout>;
};

export const getStaticProps = async () => {
  const res = await fetch("http://localhost:3000/api/cat_ram");
