...
export default function ShowProduct({ data }) {
  ...
  return (
    <Link href={`/products/${_id}`} className="mx-3">
      <div className="flex w-full justify-center items-center overflow-hidden bg-white rounded-lg shadow-lg mb-10 py-3 px-5 hover:bg-gray-50 ">
        <div className="w-1/3 bg-cover">
          <Image src={product_image} width={150} height={80} alt="image" />
        </div>

        <div className="w-2/3 p-4 md:p-4">
          <h1 className="text-xl font-bold text-gray-800">{product_name}</h1>
